    ...
    def copy_params(self, state_dict, coefficient_transfer=100):
        ...
        for name, param in state_dict.items():
            if name in own_state:
                ...
                # Replace all torch.cuda.FloatTensor with torch.FloatTensor (CPU version)
                random_tensor = (torch.FloatTensor(shape).random_(0, 100) <= coefficient_transfer).type(torch.FloatTensor)
